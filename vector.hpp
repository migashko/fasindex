...
public:
  ...
  iterator erase(iterator itr)
  {
    ...
    if ( !index_itr->erase(position) )
        throw std::out_of_range("vector::insert fatal 2");

    
    if ( index_itr->proxy_empty() )
    {
      size_type offset = index_itr - _vector_of_index.begin();
      _vector_of_index.erase(index_itr);
      index_itr = _vector_of_index.begin() + offset;
    }
    else
      ++index_itr;
    
    for ( ;index_itr!=_vector_of_index.end(); ++index_itr)
      index_itr->dec_next_index();
  ...
  void dump() const
  {
    ...
    std::for_each(
      ...
      [] (const typename vector_of_index::value_type& ilv)
      {
        ...
        std::cout << ilv.size() << ":(";
        ilv.dump();
        std::cout << ")]:"<< ilv.next_index() << ", " << std::endl;
